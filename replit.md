...
│   ├── events/
│   │   ├── event-card.tsx
│   │   ├── event-calendar-view.tsx  # カレンダー表示モード
│   │   ├── event-filters.tsx        # フィルター + 日付範囲 + 表示モード切替
│   │   ├── review-card.tsx
│   │   └── review-form.tsx
│   └── groups/
│       └── group-card.tsx
...
2. イベント検索 - 大学/団体区分/ジャンル/1人参加しやすさ/日付範囲でフィルター
   - 日付範囲フィルター（カレンダー形式の日付ピッカー、モバイルは1ヶ月、PCは2ヶ月表示）
   - カレンダー表示モード（月別カレンダー、イベントありの日にドット表示、日付選択でイベント一覧）
...
- 説明 (Text)
- 持ち物 (Text)
- 雰囲気タグ (Multi-select) - 雰囲気タグ
- 参加の流れ (Text)
- 定員 (Number)
- 画像URL (URL)
- map URL (URL) - Google MapsのURL（オプション、設定するとリンク・埋め込みに使用）
- **承認 (Checkbox)** - チェックでWebサイトに反映
